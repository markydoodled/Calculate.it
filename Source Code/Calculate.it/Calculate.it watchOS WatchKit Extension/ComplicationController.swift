...
class ComplicationController: NSObject, CLKComplicationDataSource {
    ...
    func getComplicationDescriptors(handler: @escaping ([CLKComplicationDescriptor]) -> Void) {
        let descriptors = [
            CLKComplicationDescriptor(identifier: "complication", displayName: "Calculate.it", supportedFamilies: [.circularSmall, .modularSmall, .utilitarianSmall, .utilitarianLarge, .extraLarge, .graphicCorner, .graphicCircular, .graphicExtraLarge, .modularLarge])
    ...
    private func createModularSmallTemplate(forDate date: Date) -> CLKComplicationTemplate {
        ...
        let image = CLKImageProvider(onePieceImage: UIImage(named: "Complications/Modular")!)
    ...
    private func createModularLargeTemplate(forDate date: Date) -> CLKComplicationTemplate {
        // Create the data providers.
        let titleTextProvider = CLKSimpleTextProvider(text: "Calculate.it", shortText: "Calc")
        titleTextProvider.tintColor = .init(red: 0.364, green: 0.356, blue: 0.894, alpha: 100)

        let textLine1 = CLKSimpleTextProvider(text: "Simple")
        let combinedText1 = CLKTextProvider(format: "%@", textLine1)
               
        let textLine3 = CLKSimpleTextProvider(text: "Calculator")
    ...
    private func createUtilitarianLargeTemplate(forDate date: Date) -> CLKComplicationTemplate {
        // Create the data providers.
        let solText = CLKSimpleTextProvider(text: "Calculate.it")
